...
const TIPOS = [
  ...
  { label: 'En pozo',      icon: '🏗️', slug: 'emprendimiento' },
  { label: 'Local',        icon: '🏪', slug: 'local' },
  { label: 'Oficina',      icon: '💼', slug: 'oficina' },
]

export default async function HomePage() {
  const destacadas = await getPropiedadesDestacadas(6)
  ...
  return (
    <>
      ...
      <section className="min-h-[calc(100vh-68px)] grid grid-cols-1 md:grid-cols-2">
        <div className="flex flex-col justify-center px-6 md:px-12 py-20">
          ...
          <p className="text-[16px] font-light leading-relaxed text-[#555] max-w-[420px] mb-10">
            Desarrollos propios y los mejores proyectos del mercado en un solo lugar. Acompañamos cada etapa, de la búsqueda al cierre.
          </p>
        ...
        <div className="hidden md:flex items-center justify-center bg-[#F5F4F2] relative overflow-hidden">
          ...
          <div className="absolute bottom-8 left-8 bg-white rounded-xl p-4 shadow-lg flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#D85A30] flex items-center justify-center text-white text-[18px]">🏢</div>
            <div>
              <div className="font-display font-black text-[22px] leading-none">2</div>
              <div className="text-[11px] text-[#555] mt-0.5">Edificios propios</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="bg-[#111] px-6 md:px-12 py-8">
        <form action="/propiedades" method="GET" className="flex flex-wrap gap-0 bg-white rounded-lg overflow-hidden max-w-4xl mx-auto border border-transparent">
      ...
      <section className="bg-[#F5F4F2] px-6 md:px-12 py-20">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3 max-w-3xl mx-auto">
          {TIPOS.map(t => (
            <Link key={t.slug} href={`/propiedades?tipo=${t.slug}`}
              className="border border-[#E2E0DC] rounded-xl p-5 text-center hover:border-[#D85A30] hover:bg-[#FDF3EF] transition-all flex flex-col items-center gap-2 no-underline">
              <span className="text-3xl">{t.icon}</span>
              <span className="text-[12px] font-semibold uppercase tracking-wide text-[#222]">{t.label}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* NOSOTROS */}
      <section className="bg-[#111] text-white px-6 md:px-12 py-24 grid grid-cols-1 md:grid-cols-2 gap-16 items-center" id="nosotros">
        <div className="hidden md:flex items-center justify-center rounded-xl bg-white/5 aspect-[4/3] text-[80px] opacity-10">🏙️</div>
        <div>
          <div className="flex items-center gap-2 mb-3">
            <div className="w-5 h-0.5 bg-[#D85A30]" />
          ...
          <h2 className="font-display font-black uppercase text-[clamp(36px,4vw,52px)] leading-[0.95] tracking-tight mb-5">
            SOCIOS EN<br />TU INVERSIÓN
          </h2>
          <p className="text-[15px] font-light leading-relaxed text-white/60 mb-10">
            Somos un equipo con años de experiencia en el mercado inmobiliario de CABA y GBA. No solo comercializamos — desarrollamos nuestros propios proyectos y construimos relaciones de largo plazo con nuestros clientes.
          </p>
          <div className="grid grid-cols-2 gap-6">
            {[
              ['Proyectos propios', 'Desarrollamos edificios desde el pozo con foco en rentabilidad para el inversor.'],
              ['Red amplia', 'Acceso a propiedades exclusivas a través de nuestra red de socios en todo el país.'],
              ['Transparencia', 'Documentación clara, comisiones justas y acompañamiento en cada etapa.'],
              ['Mercado local', 'Conocemos CABA en profundidad: barrios, precios y tendencias del mercado.'],
            ].map(([title, desc]) => (
              <div key={title} className="border-t-2 border-white/10 pt-4">
                <div className="font-display font-bold text-[17px] uppercase text-white mb-1">{title}</div>
      ...
      <section className="bg-[#F5F4F2] px-6 md:px-12 py-24" id="contacto">
        <div className="max-w-xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-[15px] text-[#555] font-light">Un asesor de G&P se comunica con vos en menos de 24 horas.</p>
          </div>
